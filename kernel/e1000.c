...
e1000_init(uint32 *xregs)
{
  ...
  initlock(&e1000_lockrx, "e1000_tx");
...
e1000_transmit(struct mbuf *m)
{
  ...
  //
  //printf("To acquire lock in transmit\n");
  acquire(&e1000_lock);
  //printf("Success acquire lock in transmit\n");
  int tail = regs[E1000_TDT];
  if (!(tx_ring[tail].status & E1000_TXD_STAT_DD)) {
    //printf("-1 release lock in transmit\n");
    release(&e1000_lock);
    return -1;
  }
  ...
  release(&e1000_lock);
  //printf("release lock in transmit\n");
  return 0;
}
